...
// wrapper to compute cusp
// an optimization version that compute rgb_dir only once
LC find_cusp(float a, float b)
{
  ...
  return find_cusp(a, b, r_dir, g_dir, b_dir);
}
...
// wrapper to find gamut intersection
// an optimization version that compute rgb_dir only once
float find_gamut_intersection(float a, float b, float L1, float C1, float L0)
{
  ...
  return find_gamut_intersection(a, b, L1, C1, L0, r_dir, g_dir, b_dir);
}
